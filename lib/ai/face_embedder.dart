...
class FaceEmbedder {
  ...
  Future<List<double>> embedImage(
    ...
    if (inputShape.length != 4) {
      throw StateError(
        'Input model MobileFaceNet tidak valid: $inputShape',
      );
    }
    ...
    final input = _decodedImageToInput(
      image,
      inputWidth,
      inputHeight,
    );
  ...
  void _logModelInfo(
    ...
    debugPrint(
      'MobileFaceNet input name: ${inputTensor.name}',
    );

    debugPrint(
      'MobileFaceNet input shape: ${inputTensor.shape}',
    );

    debugPrint(
      'MobileFaceNet input type: ${inputTensor.type}',
    );

    debugPrint(
      'MobileFaceNet output name: ${outputTensor.name}',
    );

    debugPrint(
      'MobileFaceNet output shape: ${outputTensor.shape}',
    );

    debugPrint(
      'MobileFaceNet output type: ${outputTensor.type}',
    );
  }
